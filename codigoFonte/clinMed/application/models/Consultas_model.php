...
";
	...
	public function listarReceitasExpedidas(){
		...
		$sql = "SELECT a.ds_numero_consulta, a.fk_id_cliente,b.ds_nome_cliente,a.ds_data_consulta, c.pk_id_profissional,c.ds_nome_profissional
				FROM
				tb_consultas AS a
				JOIN tb_clientes AS b ON b.pk_id_cliente = a.fk_id_cliente
				JOIN tb_profissionais AS c ON c.pk_id_profissional = a.fk_id_profissional
				WHERE
				a.ds_numero_consulta IN(SELECT ds_numero_consulta FROM tb_receitasconsultas)";
		$query = $default->query($sql);
        return $query->result_array();
	}
